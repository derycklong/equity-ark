...
- **Dividend auto-refresh + unified refresh endpoint**: daily 6am scheduler calls `rebuild_dashboard_cache(force=True)` (which does prices + dividends + cache) for every known user, then pre-warms `get_networth_history()` so the dashboard chart is fresh AND instant on first load. Single `POST /api/portfolio/cache/refresh` endpoint used by the sidebar "Refresh" button and the Dividends page UI also pre-warms the chart. The old `prices/refresh` and `dividends/refresh` endpoints are removed
...
- **Admin page** (`/admin`): admin emails are configured via `ADMIN_EMAILS` in `backend/data/.env` (case-insensitive, comma-separated for multiple admins, e.g. `ADMIN_EMAILS=derycklong@gmail.com,partner@gmail.com`). `/api/auth/me` returns `is_admin: true` when the signed-in user's email is in the list; `/api/admin/users` (admin-only) returns every user with their `last_login_at`. The Admin nav link only renders for admin users. Default sort is most-recent login first; filter by email/name; admin rows are marked with a Shield badge. **`RequireAuth` re-fetches `/api/auth/me` on every route change**, so the sidebar's Admin link appears as soon as the env var is set and the user navigates — no re-login required
